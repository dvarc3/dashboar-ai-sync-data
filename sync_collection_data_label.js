...
let dbRep;
let running = false;
const es_index = `${process.env.PROJECT_ID}-data_label_set_info_index`;
const table = `${process.env.PROJECT_ID}-data_label_set_info`;
...
(async () => {
    console.log(`SYNC_COLLECTION_DATA_LABEL START`);
    setInterval(() => forloop(), 500);
})().catch(() => {
    console.log('FAILED TO START SYNC_COLLECTION_DATA_LABEL');
});
